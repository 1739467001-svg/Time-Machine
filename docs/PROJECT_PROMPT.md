...
| M2 | 体验完善 | ✅ 完成（人脸检测 / 逐张加载 / 导出分享 / 云端引擎知情同意弹窗均已就位） |
| M3 | 年龄段连贯性优化 | ⬜ 规划中 |
| M4 | 架构文档 + 完整演示 | ⬜ 规划中 |
...
- **云端引擎阻断式知情同意**：服务端按 `AGING_PROVIDER` 解析出引擎元信息（`describeEngine()`，不实例化 Provider、不下发 Key），注入前端；当引擎为云端（gemini/qwen）时，进入摄像头、上传人脸前弹出阻断式弹窗明确告知数据流向，同意后方可继续；本地 mock 引擎不弹窗。
...
1. ✅ **阻断式隐私同意弹窗（已完成，补齐了验收标准第 4 条）**：服务端 `describeEngine()` 按 `AGING_PROVIDER` 解析引擎元信息注入前端；云端引擎（gemini/qwen）在进摄像头、上传人脸前弹阻断式弹窗告知数据流向，同意后才继续，拒绝则停留于开场页；本地 mock 不弹窗。实现刻意不实例化 Provider（其构造函数缺 Key 会抛错）、也不下发任何 Key。
2. **线上真实照片端到端验收（P0，下一步）**：按 `docs/DEPLOY.md` 部署到 Vercel（先 mock 验管线，再切 qwen/gemini），用真实照片逐项核对 PRD 第 10 节验收标准，重点确认 6 段「年龄递进明显、仍能认出是同一个人」；不达标则调整 `src/lib/aging/prompt.ts` 的提示词或换更高档模型（如 `gemini-3-pro-image`）。
